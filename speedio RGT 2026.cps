/**
  ...
  FORKID {A315D718-308D-4857-A432-98E61546CDE0}
*/

description = "Brother Speedio RGT 2026";
vendor = "Brother";
...
longDescription = "Custom Brother Speedio RGT 2026 milling post based on Autodesk's generic Brother Speedio post.";
...
properties = {
  ...
  optionalStopMoveToPosition: {
    title      : "Optional Stop Move to Inspection Position",
    description: "When enabled, performs the stop positioning move before M01 optional stops.",
    group      : "preferences",
    type       : "boolean",
    value      : false,
    scope      : "post"
  },
  outputM450BeforeStops: {
    title      : "Enable M450 Beep",
  ...
  annotateGMCodes: {
    ...
    description: "Adds inline comments with quick descriptions for G and M codes to help troubleshooting.",
    group      : "formats",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  positionAtEnd: {
    ...
    description: "'Center At Door' moves the part center under spindle at end of program. 'Home' uses machine home X.",
    group      : "homePositions",
    type       : "enum",
    values     : [
      {title:"Home", id:"home"},
  ...
  separateZOnToolChange: {
    ...
    description: "When true, output G100 XY move and output Z/H in a separate block.",
    group      : "homePositions",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  toolBreakRoutine: {
    title      : "Tool break routine",
    description: "Select either the built-in Blum break check or the RGT Toolbreak subprogram. Default: RGT Toolbreak.",
    group      : "preferences",
    type       : "enum",
    values     : [
      {title:"Built in Blum", id:"builtInBlum"},
      {title:"RGT Toolbreak", id:"rgtToolbreak"}
    ],
    value      : "rgtToolbreak",
    scope      : "post"
  }
...
var settings = {
  ...
  probing: {
    ...
    probeOn                : false, // tracks active probe state for probe spin on/off macros
...
function noSpindle() {
  return isTappingCycle(currentSection) || tool.type == TOOL_PROBE;
}
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane || smoothing.cancel || state.tcpIsActive || currentSection.isMultiAxis()) {
    ...
    if (isFirstSection()) {
      ...
      if (insertToolCall || smoothing.cancel) {
        setSmoothing(false);
        if (smoothing.cancel && tool.breakControl) {
...
function protectedProbeMove(_cycle, x, y, z) {
  ...
  var cycleFeedrate = _cycle ? _cycle.feedrate : highFeedrate;
...
function writeProbeCycle(cycle, x, y, z) {
  if (!isSameDirection(currentSection.workPlane.forward, new Vector(0, 0, 1))) {
    if (!settings.probing.allowIndexingWCSProbing && currentSection.strategy == "probe") {
      error(localize("Updating WCS / work offset using probing is only supported by the CNC in the WCS frame (no tilted workplane indexing)."));
    }
  }
  ...
  if (getProperty("probingType") == "Blum") {
    writeProbeCycleBlum(cycle, x, y, z);
    return;
  }
...
function writeProbeCycleBlum(cycle, x, y, z) {
  protectedProbeMove(cycle, x, y, z);
  switch (cycleType) {
  case "probing-x":
    var edgeCoord = x + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2);
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(edgeCoord), "X" + xyzFormat.format(edgeCoord), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-y":
    var edgeCoord = y + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2);
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "J" + xyzFormat.format(edgeCoord), "Y" + xyzFormat.format(edgeCoord), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-z":
    protectedProbeMove(cycle, x, y, Math.min(z - cycle.depth + cycle.probeClearance, cycle.retract));
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "K" + xyzFormat.format(z - cycle.depth), "Z" + xyzFormat.format(z - cycle.depth), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-x-wall":
    protectedProbeMove(cycle, x, y, z);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(x), "S" + xyzFormat.format(cycle.width1), "X1", "Z" + xyzFormat.format(z - cycle.depth + (tool.diameter / 2)), "Q" + xyzFormat.format(cycle.probeOvertravel), "R" + xyzFormat.format(cycle.probeClearance), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-y-wall":
    protectedProbeMove(cycle, x, y, z);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "J" + xyzFormat.format(y), "S" + xyzFormat.format(cycle.width1), "Z" + xyzFormat.format(z - cycle.depth + (tool.diameter / 2)), "Y1", "Q" + xyzFormat.format(cycle.probeOvertravel), "R" + xyzFormat.format(cycle.probeClearance), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-x-channel":
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(x), "S" + xyzFormat.format(cycle.width1), "X1", "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-x-channel-with-island":
    protectedProbeMove(cycle, x, y, z);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(x), "R" + xyzFormat.format(-cycle.probeClearance), "S" + xyzFormat.format(cycle.width1), "Z" + xyzFormat.format(z - cycle.depth + (tool.diameter / 2)), "X1", "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-y-channel":
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "J" + xyzFormat.format(y), "S" + xyzFormat.format(cycle.width1), "Y1", "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-y-channel-with-island":
    protectedProbeMove(cycle, x, y, z);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "J" + xyzFormat.format(y), "R" + xyzFormat.format(-cycle.probeClearance), "S" + xyzFormat.format(cycle.width1), "Z" + xyzFormat.format(z - cycle.depth), "Y1", "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-circular-boss":
    protectedProbeMove(cycle, x, y, z);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(x), "J" + xyzFormat.format(y), "S" + xyzFormat.format(cycle.width1), "Z" + xyzFormat.format(z - cycle.depth), "Q" + xyzFormat.format(cycle.probeOvertravel), "R" + xyzFormat.format(cycle.probeClearance), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-circular-partial-boss":
    protectedProbeMove(cycle, x, y, z);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "H" + xyzFormat.format(ensurePositiveAngle(cycle.partialCircleAngleA)), "U" + xyzFormat.format(ensurePositiveAngle(cycle.partialCircleAngleB)), "V" + xyzFormat.format(ensurePositiveAngle(cycle.partialCircleAngleC)), "I" + xyzFormat.format(x), "J" + xyzFormat.format(y), "S" + xyzFormat.format(cycle.width1), "Z" + xyzFormat.format(z - cycle.depth), "Q" + xyzFormat.format(cycle.probeOvertravel), "R" + xyzFormat.format(cycle.probeClearance), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-circular-hole":
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(x), "J" + xyzFormat.format(y), "S" + xyzFormat.format(cycle.width1), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-circular-partial-hole":
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "H" + xyzFormat.format(ensurePositiveAngle(cycle.partialCircleAngleA)), "U" + xyzFormat.format(ensurePositiveAngle(cycle.partialCircleAngleB)), "V" + xyzFormat.format(ensurePositiveAngle(cycle.partialCircleAngleC)), "I" + xyzFormat.format(x), "J" + xyzFormat.format(y), "S" + xyzFormat.format(cycle.width1), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-circular-hole-with-island":
    protectedProbeMove(cycle, x, y, z);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(x), "J" + xyzFormat.format(y), "R" + xyzFormat.format(-cycle.probeClearance), "S" + xyzFormat.format(cycle.width1), "Q" + xyzFormat.format(cycle.probeOvertravel), "Z" + xyzFormat.format(z - cycle.depth + (tool.diameter / 2)), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-circular-partial-hole-with-island":
    protectedProbeMove(cycle, x, y, z);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "H" + xyzFormat.format(ensurePositiveAngle(cycle.partialCircleAngleA)), "U" + xyzFormat.format(ensurePositiveAngle(cycle.partialCircleAngleB)), "V" + xyzFormat.format(ensurePositiveAngle(cycle.partialCircleAngleC)), "I" + xyzFormat.format(x), "J" + xyzFormat.format(y), "R" + xyzFormat.format(-cycle.probeClearance), "S" + xyzFormat.format(cycle.width1), "Q" + xyzFormat.format(cycle.probeOvertravel), "Z" + xyzFormat.format(z - cycle.depth + (tool.diameter / 2)), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-rectangular-hole":
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    ...
    writeExtraBlumProbing(cycle);
    zOutput.reset();
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "J" + xyzFormat.format(y), "S" + xyzFormat.format(cycle.width2), "Y1", "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-rectangular-boss":
    protectedProbeMove(cycle, x, y, z);
    zOutput.reset();
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(x), "S" + xyzFormat.format(cycle.width1), "X1", "Z" + xyzFormat.format(z - cycle.depth), "Q" + xyzFormat.format(cycle.probeOvertravel), "R" + xyzFormat.format(cycle.probeClearance), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    zOutput.reset();
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "J" + xyzFormat.format(y), "S" + xyzFormat.format(cycle.width2), "Y1", "Z" + xyzFormat.format(z - cycle.depth), "Q" + xyzFormat.format(cycle.probeOvertravel), "R" + xyzFormat.format(cycle.probeClearance), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-rectangular-hole-with-island":
    protectedProbeMove(cycle, x, y, z);
    ...
    writeExtraBlumProbing(cycle);
    zOutput.reset();
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "J" + xyzFormat.format(y), "S" + xyzFormat.format(cycle.width2), "Y1", "Z" + xyzFormat.format(z - cycle.depth + (tool.diameter / 2)), "Q" + xyzFormat.format(cycle.probeOvertravel), "R" + xyzFormat.format(-cycle.probeClearance), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-inner-corner":
    var cornerX = x + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2);
    var cornerY = y + approach(cycle.approach2) * (cycle.probeClearance + tool.diameter / 2);
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(cornerX), "X" + xyzFormat.format(cornerX), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "J" + xyzFormat.format(cornerY), "Y" + xyzFormat.format(cornerY), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-xy-outer-corner":
    ...
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "I" + xyzFormat.format(x + cornerX), "J" + xyzFormat.format(y + cornerY), "X" + xyzFormat.format(x + (2 * cornerX)), "Y" + xyzFormat.format(y + (2 * cornerY)), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, true));
    writeExtraBlumProbing(cycle);
    break;
  case "probing-x-plane-angle":
    var edgeCoord = x + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2);
    ...
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "X" + xyzFormat.format(edgeCoord), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, false));
    protectedProbeMove(cycle, x, y + 0.5 * cycle.probeSpacing, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "D" + xyzFormat.format(nominalAngle), "X" + xyzFormat.format(edgeCoord), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, false));
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeExtraBlumProbing(cycle);
    if (currentSection.strategy == "probe") {
      setProbeAngleMethod();
  ...
  case "probing-y-plane-angle":
    ...
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "Y" + xyzFormat.format(edgeCoord), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, false));
    protectedProbeMove(cycle, x + 0.5 * cycle.probeSpacing, y, z - cycle.depth);
    writeBlock(gFormat.format(65), "P8700", "A1", "M3", "D" + xyzFormat.format(nominalAngle), "Y" + xyzFormat.format(edgeCoord), "Q" + xyzFormat.format(cycle.probeOvertravel), getProbingArguments(cycle, false));
    protectedProbeMove(cycle, x, y, z - cycle.depth);
    writeExtraBlumProbing(cycle);
    if (currentSection.strategy == "probe") {
      setProbeAngleMethod();
...
function writeMeasureTools() {
  ...
  if (getProperty("measureTools")) {
    ...
    if (tools.getNumberOfTools() > 0) {
      writeBlock(mFormat.format(0), formatComment(localize("Read note")));
      ...
      for (var i = 0; i < tools.getNumberOfTools(); ++i) {
        ...
        if ((t.type == TOOL_PROBE) && !measureProbe) {
          continue;
        }
        var comment = "T" + toolFormat.format(t.number) + " " +
          "D=" + xyzFormat.format(t.diameter) + " " +
          localize("CR") + "=" + xyzFormat.format(t.cornerRadius);
        if ((t.taperAngle > 0) && (t.taperAngle < Math.PI)) {
          comment += " " + localize("TAPER") + "=" + taperFormat.format(t.taperAngle) + localize("deg");
        }
        comment += " - " + getToolTypeName(t.type);
        writeComment(comment);
        writeToolMeasureBlock(t, true);
      }
      writeComment("Reload initial tool");
      writeBlock("T" + toolFormat.format(getSection(0).getTool().number), mFormat.format(6));
    }
  ...
  if (getProperty("confirmToolLengths")) {
    ...
    optionalSection = true;
    if (tools.getNumberOfTools() > 0) {
      for (var j = 0; j < tools.getNumberOfTools(); ++j) {
        var toolToCheck = tools.getTool(j);
        if (toolToCheck.type == TOOL_PROBE) {
          continue;
        }
        var camLen = xyzFormat.format(toolToCheck.bodyLength + toolToCheck.holderLength);
        var toolTableLen = "#" + (toolLenBase + toolToCheck.number);
        var seq = sequenceNumber;
        sequenceNumber += getProperty("sequenceNumberIncrement");
        writeComment("Checking lengths of tool: " + toolToCheck.number);
        writeBlock("N" + seq + " IF [" + toolTableLen + " GE " + camLen + "] GOTO " + sequenceNumber);
        writeBlock("#3006=(TOOL " + toolToCheck.number + " TOO SHORT)");
      }
...
function writeToolMeasureBlock(toolToMeasure, preMeasure) {
  var comment = measureTool ? formatComment("MEASURE TOOL") : "";
  if (!preMeasure) {
    prepareForToolCheck();
  }
  var offsetTool = (toolToMeasure.type == TOOL_MILLING_SLOT || toolToMeasure.type == TOOL_MILLING_FACE) && (toolToMeasure.diameter > measureToolMaxDiameter);
  if (getProperty("probingType") == "Renishaw") {
    writeBlock(
      gFormat.format(65),
      "P9921",
      "M22.",
      "T" + toolFormat.format(toolToMeasure.number),
      "D" + xyzFormat.format(toolToMeasure.diameter) + ".",
      comment
    );
  } else {
    writeBlock("T" + toolFormat.format(toolToMeasure.number), mFormat.format(6));
    writeBlock(mFormat.format(19));
    writeBlock(
      gFormat.format(65),
      "P8915",
      "B0.",
      "H" + toolFormat.format(toolToMeasure.number),
      conditional(offsetTool, "R" + xyzFormat.format(toolToMeasure.diameter / 2)),
      conditional(toolToMeasure.type == TOOL_MILLING_FACE && measureAllFlutes && offsetTool, "D" + toolToMeasure.numberOfFlutes),
      comment
    );
  }
  measureTool = false;
...
function restoreWCSAfterStop() {
  if (currentSection && currentSection.wcs) {
...
function onCommand(command) {
  ...
  case COMMAND_STOP:
    writeln("");
    writeComment("Manual command stop");
    moveToStopPosition();
    if (getProperty("outputM450BeforeStops")) {
      writeBlock(mFormat.format(450));
    }
  ...
  case COMMAND_OPTIONAL_STOP:
    if (getProperty("optionalStopMoveToPosition")) {
      moveToStopPosition();
    }
  ...
  case COMMAND_LOAD_TOOL:
    ...
    writeToolBlock(gFormat.format(100),
      ...
      !separateZOnToolChange ? hFormat.format(tool.lengthOffset) : undefined,
    ...
    if (separateZOnToolChange) {
      writeBlock(getOffsetCode(), zOutput.format(start.z), hFormat.format(tool.lengthOffset));
    }
  ...
  case COMMAND_BREAK_CONTROL:
    if (!toolChecked) {
      ...
      prepareForToolCheck();
      var breakRoutine = getProperty("toolBreakRoutine");
      var runBuiltInBreak = (breakRoutine == "builtInBlum");
      var runCustomP8000 = (breakRoutine == "rgtToolbreak");
...
function onSectionEnd() {
  ...
  if (!isLastSection()) {
    ...
    var breakOnToolChange = getProperty("toolAsName") ? (nextTool.description != tool.description) : (nextTool.number != tool.number);
    if (tool.breakControl && breakOnToolChange) {
      onCommand(COMMAND_BREAK_CONTROL);
    }
  }
  if (isProbeOperation()) {
...
var codeDescriptions = {
  ...
  "G43":"tool length comp on",
  "G43.4":"TCP/tool center control on",
  "G43.5":"tool vector comp on",
  ...
  "G58":"work offset G58",
  "G59":"work offset G59",
  "G54.1":"extended work offset",
  "G65":"macro call",
  "G68.2":"tilted work plane on",
  "G69":"rotation cancel",
  ...
  "G94":"feed per minute",
  "G95":"feed per rev",
  "G98":"canned cycle initial plane return",
  "G99":"canned cycle R-plane return",
  "G100":"tool call macro",
  "M0":"program stop",
  "M1":"optional stop",
  "M3":"spindle CW",
  "M4":"spindle CCW",
  ...
  "M30":"end and rewind",
  "M98":"subprogram call",
  "M211":"parts counter 1",
  "M212":"parts counter 2",
  "M213":"parts counter 3",
  "M214":"parts counter 4",
  "M260":"high accuracy mode A",
  "M269":"high accuracy mode A off",
  "M280":"high accuracy mode B",
  "M289":"high accuracy mode B off",
  "M298":"high accuracy mode M298"
};

function getCodeDescription(letter, value) {
  var key = letter + value;
  if (codeDescriptions[key]) {
    return codeDescriptions[key];
  }
  if (letter == "G") {
    return "G-code";
  }
...
function getBlockAnnotation(text) {
  ...
  while ((match = re.exec(text)) !== null) {
    ...
    found[code] = true;
    annotations.push(code + "=" + getCodeDescription(match[1], match[2]));
  }

  if (annotations.length == 0) {
    return "";
  }
  return formatComment(annotations.join("; "));
}
...
function writeBlock() {
  ...
  var annotation = getBlockAnnotation(text);
  var outputText = text + suffix + (annotation ? " " + annotation : "");
  ...
  if (getProperty("showSequenceNumbers") == "true") {
    ...
    if (optionalSection || skipBlocks) {
      writeWords2("/", prefix + sequenceNumber, outputText);
    } else {
      writeWords2(prefix + sequenceNumber, outputText);
    }
    sequenceNumber += getProperty("sequenceNumberIncrement");
    ...
    if (optionalSection || skipBlocks) {
      writeWords2("/", outputText);
    } else {
      writeWords(outputText);
    }
  }
}
...
function getRetractParameters() {
  ...
  for (var i = 0; i < _arguments.length; ++i) {
    ...
    case X:
      if (!state.retractedX) {
        words.push("X" + (typeof _xHome == "string" ? _xHome : xyzFormat.format(_xHome)));
        xOutput.reset();
...
function onMovement(movement) {
  if ((currentSection.strategy == "adaptive" || currentSection.strategy == "adaptive2d" || currentSection.strategy == "pocket2d") && getProperty("rapidTransitions")) {
    ...
    if (movement == MOVEMENT_LEAD_IN || movement == MOVEMENT_LEAD_OUT || movement == MOVEMENT_LINK_DIRECT || movement == MOVEMENT_LINK_TRANSITION) {
      setSmoothing(false);
    } else {
      setSmoothing(smoothing.isAllowed);
    }
...
function onAction(action) {
  var sText = String(action).toUpperCase().split(":");
  if (sText.length != 2) {
  ...
  case "ROTATE_WCS":
    ...
    break;
  default:
    error(localize("Invalid action parameter: ") + sText[0] + ":" + param);
    return;
  }
}
...
function setProbeAngle() {
  if (probeVariables.outputRotationCodes) {
    ...
    case "G68":
      ...
      var probeRotationVariable = getProperty("probingType") == "Blum" ? "#143" : pr;
      writeBlock(
        ...
        formatCompensationParameter("R", probeRotationVariable)
      );
