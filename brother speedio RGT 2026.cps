...
properties = {
  ...
  blumUseA1: {
    title      : "Blum A1 parameter",
    description: "Outputs A1 in BLUM probing and inspection cycles.",
    group      : "probing",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  blumMacroStyle: {
    title      : "Blum macro style",
    description: "Select BLUM probing macro argument style.",
    group      : "probing",
    type       : "enum",
    values     : [
      {title:"Modern (2026)", id:"modern"},
      {title:"Legacy (RGTSPD)", id:"legacyRGT"}
    ],
    value: "modern",
    scope: "post"
  },
  ...
  toolBreakControlMacro: {
    title      : "Tool break macro",
    description: "Subprogram number called by M98 when Tool Break Control is requested and M298 L0 is output.",
    group      : "preferences",
    type       : "integer",
    value      : 8000,
    scope      : "post"
...
var pendingToolBreakControlCall = false;
...
function setSmoothing(mode) {
  ...
  default:
    ...
    if (!mode && pendingToolBreakControlCall) {
      var toolBreakControlMacro = Math.max(1, Math.floor(getProperty("toolBreakControlMacro")));
      writeBlock(mFormat.format(98), "P" + toolBreakControlMacro);
      pendingToolBreakControlCall = false;
    }
    break;
  }
...
function useBlumA1() {
  return (getProperty("probingType") == "Blum") && getProperty("blumUseA1");
}

function useLegacyBlumMacros() {
  return (getProperty("probingType") == "Blum") && (getProperty("blumMacroStyle") == "legacyRGT");
}

var legacyProbeOutputWorkOffset = 1;

function onParameter(name, value) {
  if (name == "probe-output-work-offset") {
    legacyProbeOutputWorkOffset = (value > 0) ? value : 1;
  }
}
...
function getProbingArguments(cycle, updateWCS) {
  ...
  var probeOutputWorkOffset = useLegacyBlumMacros() ? legacyProbeOutputWorkOffset : currentSection.probeWorkOffset;
  if (outputWCSCode) {
...
function onCommand(command) {
  ...
  case COMMAND_BREAK_CONTROL:
    pendingToolBreakControlCall = true;
    return;
